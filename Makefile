...
test: setup
	@poetry run pytest --durations=3 -v --cov=${PWD}/asrecognition 

test_report: setup
	@poetry run pytest --durations=3 -v --cov=${PWD}/asrecognition --cov-report xml:reports/coverage.xml --junitxml=reports/tests.xml
